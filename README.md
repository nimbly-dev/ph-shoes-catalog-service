...
- **Dockerfile**: development and production container recipe (dev target + runtime stage).
...
1. Build everything once (required before running the dev container):
   ...
   ```
2. Ensure `ph-shoes-services/.env` has the Snowflake credentials required by the JPA datasource.
